...
public class PlayerController : MonoBehaviour
{
    ...
    [SerializeField]
    [Tooltip("A list of all the attacks and information about them")]
    private PlayerAttackInfo[] m_Attacks;

    [SerializeField]
    [Tooltip("Amount of healh that the player start with")]
    ...
    //we declare all our private variables here

    //The current move direction of the player. Does NOT include magnitude
    private Vector2 p_Velocity;

    //in order to do anything we cannot be frozen
    private float p_FrozenTimer;

    //default color. Cached so we can switch betwee colors
    ...
    //functions called first and only once per object (like start)
    //awkae runs FIRST before any function (even start)
    ...
    private void Update()
    {
        ...
        //updating velocity
        //if a player doesn't press hard enough on a key we don't want them to move
    ...
    //for physics based, we want to use the following (runs on a set time frame)
    //frame rates can be variable between systems
    //fixed update runs on set time frame
